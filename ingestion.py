...
async def index_documents_async(documents: List[Document], batch_size: int = 50):
    ...
    log_info(f"📚️ VectorStore Indexing: Preparing to add {len(documents)} documents to vector store", Colors.DARKCYAN)
    # Create batches
    batches = [documents[i : i + batch_size] for i in range(0, len(documents), batch_size)]
    log_info(f"📦️ VectorStore Indexing: Split into {len(batches)} batches of {batch_size} documents each")

    # Process all batces concurrently
...
async def main():
    ...
    log_info("🗺️  TavilyCrawl: Starting to Crawl documentation from https://python.langchain.com/", Colors.PURPLE)
    ...
    res = tavily_crawl.invoke({
        ...
        # "instructions": "content on ai agents", # Instructions to filter the content on the topic we want
    })
    ...
    log_info(f"✂️ Text Splitter: Processing {len(all_docs)} documents with 400 chunk size and 200 overlap", Colors.YELLOW)
    ...
    log_success("🎉 Documentation ingestion pipelinefinished successfully!")
